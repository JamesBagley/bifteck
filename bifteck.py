...
class XptFile:
    ...
    for i in range(num_timepoints):
        ...
        if ts_offset + 8 > len(footer):
            raise Warning(f"Timestamp {i} out of bounds at {file_path}, {stream_path}, using fallback of 1 second intervals")
        ...
        if not (1 < ts_float < 100000):
            ...
            if timestamps:
                ...
                raise ValueError(f"First timestamp is invalid: {ts_float}, {file_path}, {stream_path}")
            continue
...
def read_xpt_file(file_path):
    with olefile.OleFileIO(file_path) as ole:
        ...
        for stream in data_streams:
            ...
            try:
                df = read_xpt_stream(ole, stream_path=stream).assign(File=file_path, Subset=subset_number)
            except Exception as e:
                raise Warning(f"  [!] Error processing {file_path} subset {subset_number},  skipping: {e}")
                continue
        frames.append(df)
    if not frames:
        raise ValueError(f"No valid data extracted from {file_path}")
    
    return pd.concat(frames, ignore_index=True).melt(id_vars=["File", "Subset", "Time", "Time_Abs", "Temperature"], var_name="Well", value_name="Measurement")